            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    using (var _quotationDataContext = new QuotationDataContext())
                    {
                        ...
                        _LeaveRequest.VacationDays = (item.VacationDays == null ? Utilities.GetTotalWorkingDays(_LeaveRequest.StartDate.Value, _LeaveRequest.EndDate.Value) * (item.VacationHours / 8m) : item.VacationDays);
                        ...
                        Utilities.SendMail(
                             ...
                             saved.RequestDate.GetValueOrDefault().ToString("dd/MM/yyyy HH:mm"),
                             saved.LeaveType1.Description,
                             saved.StartDate.GetValueOrDefault().ToString("dd/MM/yyyy"),
                             ...
                             saved.StatusDate.GetValueOrDefault().ToString("dd/MM/yyyy HH:mm"),
