    ...
    public partial class Quotation
    {
        ...
        public string NoteFromProduction
        {
            get
            {
                ...
                {
                    ProductionOrder po = db.ProductionOrders.OrderByDescending(p => p.ID).FirstOrDefault(s => s.Quotation.ID == this.ID);
                    //List<ProductionOrderTechSpec> tss = db.ProductionOrderTechSpecs.OrderBy(p => p.ID).Where(s => s.ProductionOrder.ID == po.ID && s.Status == 17).OrderBy(d => d.ID).ToList();
                    List<ProductionOrderTechSpec> tss = db.ProductionOrderTechSpecs.OrderBy(p => p.ID).Where(s => s.ProductionOrder.ID == po.ID).OrderBy(d => d.ID).ToList();
                    if (tss != null)
                    {
