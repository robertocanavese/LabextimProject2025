...
namespace LabExtim
{
    public partial class ProductionOrderDetailsInsertByOdP : ProductionOrderDetailsInsertController
    {
        ...
        public int CurProductionOrderId
        {
            get
            {
                if (ViewState["CurProductionOrderId"] != null)
                    return Convert.ToInt32(ViewState["CurProductionOrderId"]);
                return -1;
            }
            set { ViewState["CurProductionOrderId"] = value; }
        }
        ...
        {
            var _tempOdP = -1;
            Int32.TryParse(itbNoOdP.Text, out _tempOdP);
            var _found = false;
            lblInfo.Text = GetProductionOrderToolTip(_tempOdP.ToString(), out _found);
            lblInfo.Attributes.Remove("class");
            if (_found)
            {
                lblInfo.Attributes.Add("class", "bold");
            }
            if (!_found)
            {
                lblInfo.Attributes.Add("class", "red");
            }
            lbtNewItem.Enabled = _found;
            lbtSubmit.Enabled = _found;
            //GetGridData();
            if (_found)
            {
                CurProductionOrderId = _tempOdP;
                var _temp = new List<TempProductionOrderDetail>();
                _temp.AddRange(GetProductionOrderDetailsOfAnOdPId(_tempOdP));
                ...
                //grdProductionOrderDetails.Columns[4].Visible = (ddlEmployees.SelectedValue == "" || ddlEmployees.SelectedValue == "0");
        ...
        protected void ddlRawMaterialSup_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (_ddlRawMaterialSup.SelectedValue != "P0")
            {
                ...
                //TempProductionOrderDetails[i].SFlag = _ddlRawMaterialSup.SelectedValue.Substring(0, 1);
        ...
        {
            for (var i = 0; i < 5; i++)
                InsertRow(CurProductionOrderId, CurrentCompanyId);
            grdProductionOrderDetails.DataSource = TempProductionOrderDetails;
            grdProductionOrderDetails.DataBind();
        ...
        protected void ldsOfaDropDownList_Selecting(object sender, LinqDataSourceSelectEventArgs e)
        {
            ...
            if (((LinqDataSourceView)sender).TableName == "Employees")
            {
                using (var _ctx = new QuotationDataContext())
                {
                    e.Result = _ctx.Employees.Where(p => p.ID_Company == CurrentCompanyId && p.LeavingDate > DateTime.Today).OrderBy(p => p.UniqueName).ToArray();
                }
            }
        ...
        protected void grdProductionOrderDetails_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow && !e.Row.RowState.ToString().Contains("Edit"))
            {
                try
                {
                    ...
                    //Label _txtID_ProductionOrder = (Label)e.Row.FindControl("txtID_ProductionOrder");
                    ////_txtID_ProductionOrder.Text = ((TempProductionOrderDetail)e.Row.DataItem).ID_ProductionOrder.ToString();
                    //_txtID_ProductionOrder.ToolTip = GetProductionOrderToolTip(_txtID_ProductionOrder.Text);
                    ...
                    //((LinkButton)e.Row.FindControl("SubmitLinkButton")).Visible = (ddlEmployees.SelectedValue != "" && ddlEmployees.SelectedValue != "0");
                    //((LinkButton)e.Row.FindControl("EditLinkButton")).Visible = (ddlEmployees.SelectedValue != "" && ddlEmployees.SelectedValue != "0");
                    //((LinkButton)e.Row.Cells[2].Controls[0]).Visible = (ddlEmployees.SelectedValue != "" && ddlEmployees.SelectedValue != "0");
                    //((LinkButton)e.Row.FindControl("DeleteLinkButton")).Visible = (ddlEmployees.SelectedValue != "" && ddlEmployees.SelectedValue != "0");
                }
                catch
                {
                }
            ...
            if (e.Row.RowType == DataControlRowType.DataRow && e.Row.RowState.ToString().Contains("Edit"))
            {
                ...
                //    Label _lblState = (Label)_curRow.FindControl("lblState");
                //    _lblState.Text = ((TempProductionOrderDetail)_curItem).State == TempProductionOrderDetail.ItemState.Error ? "???" : "OK";

                var _lblID_ProductionOrder = (Label)e.Row.FindControl("lblID_ProductionOrder");
                if (_lblID_ProductionOrder != null)
                {
                    //_txtID_ProductionOrder.Text = ((TempProductionOrderDetail)_curItem).ID_ProductionOrder.ToString();
                    //_lblID_ProductionOrder.ToolTip = GetProductionOrderToolTip(_lblID_ProductionOrder.Text);
                }

                var _ddlEmployee = (DropDownList)e.Row.FindControl("ddlEmployee");
                if (_ddlEmployee != null)
                {
                    if (((TempProductionOrderDetail)e.Row.DataItem).ID_Owner != null)
                        _ddlEmployee.SelectedValue = ((TempProductionOrderDetail)e.Row.DataItem).ID_Owner.ToString();
                }

                var _txtProductionDate = (TextBox)e.Row.FindControl("txtProductionDate");
                if (_txtProductionDate != null)
                {
                    if (((TempProductionOrderDetail)e.Row.DataItem).ProductionDate != null)
                        _txtProductionDate.Text =
                            ((TempProductionOrderDetail)e.Row.DataItem).ProductionDate.Value.ToString("dd/MM/yyyy");
                }
        ...
        protected void mnuOperations_MenuItemClick(object sender, MenuEventArgs e)
        {
            ...
            using (var QuotationDataContext = new QuotationDataContext())
            {
                ...
                if (CurMenuTarget == 0)
                {
                    ...
                    TempProductionOrderDetails[_clientPos].RFlag = (_cbk.Prefix == "M" ? _cbk.Prefix : null); // _cbk.Prefix;
        ...
        public void UpdateDataSource(int rowIndex)
        {
            ...
            try
            {
                ...
                var _lblID_ProductionOrder =
                    (Label)grdProductionOrderDetails.Rows[i].FindControl("lblID_ProductionOrder");
                if (_lblID_ProductionOrder.Text != string.Empty)
                    TempProductionOrderDetails[i].ID_ProductionOrder = Convert.ToInt32(_lblID_ProductionOrder.Text);

                var _ddlEmployee = (DropDownList)grdProductionOrderDetails.Rows[i].FindControl("ddlEmployee");
                if (_ddlEmployee.SelectedValue != "0")
                    TempProductionOrderDetails[i].ID_Owner = Convert.ToInt32(_ddlEmployee.SelectedValue);

                var _txtProductionDate =
                    (TextBox)grdProductionOrderDetails.Rows[i].FindControl("txtProductionDate");

                var _tempDate = DateTime.Today;
                if (DateTime.TryParse(_txtProductionDate.Text, out _tempDate))
                {
                    TempProductionOrderDetails[i].ProductionDate = _tempDate;
                }
