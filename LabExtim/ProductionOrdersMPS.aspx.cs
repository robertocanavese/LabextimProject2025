...
namespace LabExtim
{
    public partial class ProductionOrdersMPS : ProductionOrderController
    {
        ...
        public void SetFilter()
        {
            ...
            {
                ldsProductionMPS.WhereParameters.Add("ID_Customer", DbType.Int32, senMain.DropDownList2.SelectedValue);
                _filter += " AND ID_Customer == @ID_Customer";
            }
            if (senMain.DropDownList3.SelectedValue != string.Empty)
            {
                ldsProductionMPS.WhereParameters.Add("IDProductionMachine", DbType.Int32, senMain.DropDownList3.SelectedValue);
                _filter += " AND IDProductionMachine == @IDProductionMachine";
            }
            if (senMain.DropDownList4.SelectedValue != string.Empty)
            {
                ldsProductionMPS.WhereParameters.Add("NumProductionMachine", DbType.Int32, senMain.DropDownList4.SelectedValue);
        ...
        private void PopulateSearchEngine()
        {
            ...
            using (var _qc = new QuotationDataContext())
            {
                ...
                senMain.LblDropDownList2Text = "Cliente";
                var _customerItems =
                    _qc.Customers.OrderBy(d => d.Name).Select(s => new ListItem { Text = s.Name, Value = s.Code.ToString() }).ToArray();
                senMain.DropDownList2.Items.AddRange(_customerItems);
                senMain.DropDownList2.Items.Insert(0, new ListItem("Tutti", ""));

                senMain.LblDropDownList3Text = "Macchina";
                var _machineItems =
                    _qc.ProductionMachines.OrderBy(d => d.Description).Select(s => new ListItem { Text = s.Description, Value = s.ID.ToString() }).ToArray();
                senMain.DropDownList3.Items.AddRange(_machineItems);
                senMain.DropDownList3.Items.Insert(0, new ListItem("Tutte", ""));

                senMain.LblDropDownList4Text = "Numero macchina";
                senMain.DropDownList4.Items.Insert(0, new ListItem("Tutte", ""));
                senMain.DropDownList4.Items.Insert(1, new ListItem("0", "0"));
                senMain.DropDownList4.Items.Insert(2, new ListItem("1", "1"));
                senMain.DropDownList4.Items.Insert(3, new ListItem("2", "2"));
        ...
        protected void grdProductionMPS_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                _hypEdit.Attributes.Add("onclick",
                    ...
                    ((VW_ProductionMP) e.Row.DataItem).IDProductionOrder + "')");
                ...
                //}
                var _CloseLinkButton = (LinkButton) e.Row.Cells[1].FindControl("CloseLinkButton");
                _CloseLinkButton.Visible = (((VW_ProductionMP)e.Row.DataItem).poStatus != 3);

                var _ClosePhaseLinkButton = (LinkButton)e.Row.Cells[14].FindControl("ClosePhaseLinkButton");
                _ClosePhaseLinkButton.Visible = (((VW_ProductionMP)e.Row.DataItem).Status== 11);

                var _ReopenPhaseLinButton = (LinkButton)e.Row.Cells[14].FindControl("ReopenPhaseLinButton");
                _ReopenPhaseLinButton.Visible = (((VW_ProductionMP)e.Row.DataItem).Status == 12);
        ...
        protected void ldsProductionMPS_Selecting(object sender, LinqDataSourceSelectEventArgs e)
        {
            ...
            e.Result = _qc.VW_ProductionMPs.OrderBy(qt => qt.ProdStart);
            ...
            //        ldsProductionMPS.OrderByParameters.Clear();
            //        ldsProductionMPS.AutoGenerateOrderByClause = false;
            //        e.Result = _qc.VW_ProductionMPs.OrderBy(qt => qt.mpstDescription);
            //        break;
            //    case ("CustomerName"):
            //        ldsProductionMPS.OrderByParameters.Clear();
            //        ldsProductionMPS.AutoGenerateOrderByClause = false;
            //        e.Result = _qc.VW_ProductionMPs.OrderBy(qt => qt.cuName);
            //        break;
            //    case ("StartDate"):
            //        ldsProductionMPS.OrderByParameters.Clear();
            //        ldsProductionMPS.AutoGenerateOrderByClause = false;
            //        e.Result = _qc.VW_ProductionMPs.OrderByDescending(qt => qt.ProdStart).ThenByDescending(qt => qt.ID);
            //        break;
        ...
        protected void grdProductionMPS_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (e.CommandName == "ClosePhase")
            {
                var table = ldsProductionMPS.GetTable();
                using (var _qc = (QuotationDataContext)table.CreateContext())
                {
                    var _toDeleteProductionOrder = _qc.ProductionMPs.FirstOrDefault(po => po.IDProductionOrder == _destinationId);
                    ProductionOrderService.CloseProductionOrderSchedule(_qc, _toDeleteProductionOrder.ID, Convert.ToInt32(e.CommandArgument.ToString().Split('|')[1]));
                    _qc.SubmitChanges();
                }
                SetFilter();
                grdProductionMPS.DataBind();
            ...
            if (e.CommandName == "ReopenPhase")
            {
                var table = ldsProductionMPS.GetTable();
                using (var _qc = (QuotationDataContext)table.CreateContext())
                {
                    var _toDeleteProductionOrder = _qc.ProductionMPs.FirstOrDefault(po => po.IDProductionOrder == _destinationId);
                    ProductionOrderService.ReopenProductionOrderSchedule(_qc, _toDeleteProductionOrder.ProductionOrder, Convert.ToInt32(e.CommandArgument.ToString().Split('|')[1]));
                    _qc.SubmitChanges();
                }
                SetFilter();
                grdProductionMPS.DataBind();
    ...
    public class CaseInsensitiveComparer12 : IComparer<string>
    {
