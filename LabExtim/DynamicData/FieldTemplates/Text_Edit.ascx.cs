        ...
        protected void CustomValidator1_ServerValidate(object source, ServerValidateEventArgs args)
        {
            if (Mode == DataBoundControlMode.Insert)
            {
                using (var _qc = new QuotationDataContext())
                {
                    if (Table.Name == "Quotations" && Column.Name == "Subject")
                        args.IsValid =
                            _qc.Quotations.FirstOrDefault(
                                t => t.Subject.Replace(" ", "").Contains(args.Value.Replace(" ", ""))) == null;
                    ...
                    else if (Table.Name == "MacroItems" && Column.Name == "Link")
                    {
                        args.IsValid = string.IsNullOrEmpty(args.Value) ||
                        _qc.MacroItems.FirstOrDefault(
                            t => t.ID.ToString() == args.Value) != null;
                    }
                    else if (Table.Name == "MacroItems" && Column.Name == "PILink")
                    {
                        args.IsValid = string.IsNullOrEmpty(args.Value) ||
                        _qc.PickingItems.FirstOrDefault(
                            t => t.ID.ToString() == args.Value) != null;
                    }
                    else if (Table.Name == "PickingItems" && Column.Name == "MILink")
                    {
                        args.IsValid = string.IsNullOrEmpty(args.Value) ||
                        _qc.MacroItems.FirstOrDefault(
                            t => t.ID.ToString() == args.Value) != null;
                    }
                    else if (Table.Name == "PickingItems" && Column.Name == "Link")
                    {
                        args.IsValid = string.IsNullOrEmpty(args.Value) ||
                        _qc.PickingItems.FirstOrDefault(
                            t => t.ID.ToString() == args.Value) != null;
            ...
            else
            {
                using (var _qc = new QuotationDataContext())
                {
                    if (Table.Name == "Quotations" && Column.Name == "Subject")
                        args.IsValid =
                            _qc.Quotations.Where(t => t.Subject.Replace(" ", "").Contains(args.Value.Replace(" ", "")))
                                .Count() <= 1;
                    else if (Table.Name == "QuotationTemplates" && Column.Name == "Description")
                        args.IsValid =
